...
public:
  /**
   * Computes the score between two std::vectors. std::vectors must be sorted and 
 ...
 * @param MUSTNORMALIZE if std::vectors must be normalized to compute the score
...
#define __SCORING_CLASS(NAME, MUSTNORMALIZE, NORM) \
  ...
  { public: \
    /** \
     * Computes score between two std::vectors \
     * @param v \
     * @param w \
     ...
     * @return true iff std::vectors must be normalized \
     */ \
