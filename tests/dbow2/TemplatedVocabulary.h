...
namespace DBoW2
{
		...
		const std::vector<pDescriptor> &descriptors, int current_level)
	{
		...
		std::vector<std::vector<size_t>> groups; // groups[i] = [j1, j2, ...]
		...
		if (current_level < m_L)
		{
			...
			for (unsigned int i = 0; i < clusters.size(); ++i)
			{
				...
				for (auto vit = groups[i].begin(); vit != groups[i].end(); ++vit)
				{
		...
		const std::vector<pDescriptor> &pfeatures, std::vector<TDescriptor> &clusters) const
	{
		...
		size_t ifeature = rand() % pfeatures.size();
		...
		const std::vector<TDescriptor> &features, BowVector &v) const
	{
		...
		if (m_weighting == TF || m_weighting == TF_IDF)
		{
			...
			if (!v.empty() && !must)
			{
				...
				const size_t nd = v.size();
				for (auto vit = v.begin(); vit != v.end(); vit++)
					vit->second /= (double)nd;
			}
		}
		else // IDF || BINARY
		{
	...
	bool TemplatedVocabulary<TDescriptor, F>::loadFromTextFile(const std::string &filename)
	{
		...
		// nodes
		size_t expected_nodes = (size_t)((pow((double)m_k, (double)m_L + 1) - 1) / (m_k - 1));
		m_nodes.reserve(expected_nodes);
		size_t expected_words = (size_t)pow((double)m_k, (double)m_L + 1);
