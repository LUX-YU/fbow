...
namespace fbow
{
    ...
    void VocabularyCreator::create(fbow::Vocabulary &Voc, const std::vector<cv::Mat> &features, const std::string &desc_name, Params params)
    {
    ...
    void VocabularyCreator::createLevel(int parent, int curL, bool recursive)
    {
        ...
        else
        {
            ...
            {
                id_assigments.create(
                    parent * _params.k + 1 + (uint32_t)i,
                    findices.size() / _params.k
                );

                assigments_ref.push_back(
                    id_assigments[parent * _params.k + 1 + (uint32_t)i]
                );
            }
        ...
        new_nodes.reserve(center_features.size());
        {
            ...
            {
                uint32_t id     = parent * _params.k + 1 + (uint32_t)c;
                uint32_t findex = findices.size() == center_features.size() ? findices[c] : std::numeric_limits<uint32_t>::max();
                new_nodes.push_back(Node(id, parent, center_features[c], findex));
            }
        ...
        if (((_params.L != -1 && curL < (_params.L - 1)) || _params.L == -1) && assigments_ref.size() > 0)
        {
            ...
            { // recursive mode(one thread only)
                for (size_t i = 0; i < new_nodes.size(); i++)
                    createLevel(parent * _params.k + 1 + (uint32_t)i, curL + 1);
            }
            else
            { 
                ...
                for (size_t i = 0; i < new_nodes.size(); i++)
                {
                    ParentDepth_ProcesQueue.push(
                        std::make_pair((int)(parent * _params.k + 1 + i), (int)(curL + 1))
                    );
                }
            }
        }
...
#pragma omp parallel for
    ...
    cv::Mat VocabularyCreator::meanValue_binary(const std::vector<uint32_t> &indices)
    {
        ...
        cv::Mat mean = cv::Mat::zeros(1, (int)_descNBytes, CV_8U);
    ...
    {
        cv::Mat mean(1, (int)_descCols, (int)_descType);
    ...
    void VocabularyCreator::convertIntoVoc(Vocabulary &Voc, std::string desc_name)
    {
        ...
        if (_descType == CV_32F)
            aligment = 32;
        Voc.setParams(aligment, _params.k, (int)_descType, (int)_descNBytes, nonLeafNodes, desc_name);
