...
std::vector<std::string> readImagePathsFromFile(char *txtlist)
{
...
std::vector<cv::Mat> loadFeatures(const std::vector<std::string> &path_to_images, std::string descriptor = "")
{
    // select detector
    ...
    for (size_t i = 0; i < path_to_images.size(); ++i)
    {
        ...
        std::cout << "done detecting features (" << i << "/" << path_to_images.size() << ")" << std::endl;
    }
    return features;
}
